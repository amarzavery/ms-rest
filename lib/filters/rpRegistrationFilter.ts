...
export class RPRegistrationFilter extends BaseFilter {
  ...
  constructor(retryTimeout: number = 30) {
    super();
   ...
   */
  getRequestEssentials(originalRequest: WebResource, reuseUrlToo: boolean = false): any {
    let reqOptions: any = {
      headers: {}
    };
    if (reuseUrlToo) {
  ...
  async getRegistrationStatus(url: string, originalRequest: WebResource): Promise<boolean> {
    let reqOptions: any = this.getRequestEssentials(originalRequest);
    let res: HttpOperationResponse;
    let result = false;
    reqOptions.url = url;
